...
const QUOTES = [
  { text: 'Every champion was once a contender that refused to give up.', author: 'Champions Park', lang: 'en' },
...
export default function HomePage() {
  ...
  const trustBadges = [
    { icon: '⚡', text: 'Instant Plan Generation' },
    { icon: '🔒', text: 'Secure & Private' },
    { icon: '🏆', text: '300 Beta Spots Only' },
  ];

  if (!mounted) return null;

  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #E8F5E9 0%, #F1F8E9 50%, #E8F5E9 100%)', fontFamily: 'Georgia, system-ui, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>

      {/* ROTATING QUOTES HEADER */}
      <div style={{ width: '100%', background: '#2D5A2D', padding: '14px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ opacity: quoteVisible ? 1 : 0, transition: 'opacity 0.5s ease', direction: isRTL ? 'rtl' : 'ltr' }}>
          <p style={{ fontSize: '14px', color: 'rgba(253,252,250,0.95)', fontStyle: 'italic', margin: 0, fontFamily: isRTL ? 'system-ui, sans-serif' : 'Georgia, serif', lineHeight: '1.5' }}>
            "{currentQuote.text}"
          </p>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', marginTop: '8px' }}>
          {QUOTES.map((_, i) => (
            <div key={i} onClick={() => setQuoteIndex(i)} style={{ width: '5px', height: '5px', borderRadius: '50%', background: i === quoteIndex ? '#86A886' : 'rgba(134,168,134,0.3)', transition: 'background 0.3s', cursor: 'pointer' }} />
          ))}
        </div>
      </div>
              ...
              <form onSubmit={handleSignup}>
                {[
                  ...
                  <div key={field.key} style={{ marginBottom: '14px' }}>
                    ...
                    <input type={field.type} value={signupForm[field.key]} onChange={e => setSignupForm({ ...signupForm, [field.key]: e.target.value })} placeholder={field.placeholder} style={{ width: '100%', padding: '12px 14px', background: '#FDFCFA', border: '1.5px solid rgba(134,168,134,0.35)', borderRadius: '10px', color: '#1B3A2A', fontSize: '15px', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                  </div>
                ))}

                <div style={{ marginBottom: '22px' }}>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', letterSpacing: '1px', textTransform: 'uppercase', color: '#5A7A5A', marginBottom: '10px' }}>Your Fitness Goal</label>
                  {goalOptions.map(opt => (
                ...
                <p style={{ textAlign: 'center', fontSize: '12px', color: '#5A7A5A', marginTop: '12px' }}>Free forever during beta. No credit card required.</p>
              </form>
            )}
