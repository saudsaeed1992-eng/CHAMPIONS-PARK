...
export default function OnboardingPage() {
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      // Upload only photos that exist — all optional
      await Promise.all([
        photos.front ? uploadPhoto('front', photos.front) : Promise.resolve(),
        photos.back ? uploadPhoto('back', photos.back) : Promise.resolve(),
        photos.left ? uploadPhoto('left', photos.left) : Promise.resolve(),
        photos.right ? uploadPhoto('right', photos.right) : Promise.resolve(),
      ...
      {generating && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#86A886' }} />
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: '580px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '8px' }}>
            {[{ n: 1, label: 'Profile' }, { n: 2, label: 'Equipment' }, { n: 3, label: 'Photos' }, { n: 4, label: 'Schedule' }].map(({ n, label }) => (
              <div key={n} style={{
        ...
        <div style={{
          ...
          {step === 1 && (
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#5A7A5A', marginBottom: '20px', fontStyle: 'italic' }}>{t('stepProfileSubtitle')}</p>
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px', marginBottom: '16px' }}>
                {equipmentOptions.map(opt => (
                  <label key={opt.id} style={{
                    ...
                    borderRadius: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '500', color: '#1B3A2A', transition: 'all 0.2s',
                  }}>
              ...
              <label style={{
                ...
                borderRadius: '12px', cursor: 'pointer', minHeight: '100px', overflow: 'hidden', marginBottom: '20px',
              }}>
          ...
          {step === 3 && (
            <div>
              ...
              <div style={{ background: 'rgba(45,90,45,0.06)', border: '1px solid rgba(45,90,45,0.15)', borderRadius: '10px', padding: '10px 14px', marginBottom: '16px', fontSize: '12px', color: '#2D5A2D' }}>
                📸 Photos are optional — you can skip and add them later from your dashboard
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
                {photoTypes.map(({ key, label }) => (
                  <label key={key} style={{
                    ...
                    borderRadius: '12px', cursor: 'pointer', minHeight: '120px', overflow: 'hidden', transition: 'all 0.2s',
                  }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => setStep(4)} style={{ flex: 2, padding: '13px', background: '#2D5A2D', border: 'none', borderRadius: '12px', color: '#FDFCFA', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.3s' }}>
                  Continue → Schedule ⏰
                </button>
              </div>
            </div>
          ...
          {step === 4 && (
            <div>
              ...
              <p style={{ fontSize: '13px', color: '#5A7A5A', marginBottom: '20px', fontStyle: 'italic' }}>{t('stepScheduleSubtitle')}</p>
              ...
              <div style={sectionStyle}>
                ...
                <div style={{ marginBottom: '10px' }}>
                  ...
                  <textarea style={{ ...inputStyle, height: '90px', resize: 'vertical' }} value={schedule.daily_routine} onChange={e => setSchedule({ ...schedule, daily_routine: e.target.value })} placeholder="e.g. I wake at 6am, work 9-5, prefer morning workouts..." />
                </div>
                <div>
                  <label style={labelStyle}>{t('additionalNotes')}</label>
                  <textarea style={{ ...inputStyle, height: '70px', resize: 'vertical' }} value={schedule.notes} onChange={e => setSchedule({ ...schedule, notes: e.target.value })} placeholder={t('additionalNotesPlaceholder')} />
                </div>
              </div>
